    ...
    public class glTfGhConverterComponent : GH_Component
    {
        ...
        private void SetMaterials(glTFLoader.Schema.Gltf gltf, List<int> materialIndices, List<string> materialSpecs)
        {
            ...
            for (int i = 0; i < interimMaterials.Count; i++)
            {
                ...
                // None of the following material properties export. We use a post-process string replacement to add the emissiveFactor to the gltf
                //mat.EmissiveFactor = new float[] {.5f,.5f,.5f};
                //mat.Name = "Material" + i;
                //mat.AlphaCutoff = .3f;
